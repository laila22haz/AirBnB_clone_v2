...
def do_deploy(archive_path):
    ...
    try:
        ...
        new_folder = '/data/web_static/releases/'.format(archive_ext)
        put(archive_name, '/temp')
        run('tar -xzf /tmp/{} -C /data/web_static/releases/{}'.format(archive_name, archive_ext))
        run('rm -rf /temp/{}'.format(archive_name))
        run('rm -rf {}'.format('/data/web_static/current'))
        run('ln -s /data/web_static/releases/{}/' '/data/web_static/current'.format(archive_ext))
